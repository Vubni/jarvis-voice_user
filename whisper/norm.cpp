...
void recognition_thread() {
    ...
    const size_t MIN_AUDIO_LENGTH = 16000; // Минимум 1 секунда аудио
    const size_t MAX_AUDIO_LENGTH = 16000 * 9; // Максимум 5 секунд (для контекста)
    ...
    while (!stop_flag) {
        ...
        if (!audio_data.empty() && whisper_ctx) {
            // Преобразование в float и добавление в буфер
            ...
            if (accumulated_audio.size() > MAX_AUDIO_LENGTH) {
                accumulated_audio.erase(accumulated_audio.begin(), 
                    accumulated_audio.begin() + (accumulated_audio.size() - MAX_AUDIO_LENGTH));
            }
            ...
            if (elapsed_ms >= 200 && accumulated_audio.size() >= MIN_AUDIO_LENGTH) {
                ...
                last_recognition_time = now; // Обновление времени последнего распознавания
            }
        }
    }
}
...
int main() {
    ...
    whisper_ctx = whisper_init_from_file_with_params("models/small.bin", cparams);
    if (!whisper_ctx) {
