...
void replaceCheckBox(QWidget* parent, const QString& layout_name, const QString& object_name) {
    if (!parent) {
        qWarning() << "Parent widget is null!";
        return;
    }

    // Находим layout по имени
    QHBoxLayout *layout = parent->findChild<QHBoxLayout*>(layout_name);
    if (!layout) {
        qWarning() << "Layout" << layout_name << "not found!";
        return;
    }

    // Проверяем, не был ли уже заменен виджет
    if (parent->findChild<Switch*>(object_name)) {
        return; // Уже заменен
    }

    // Ищем старый QCheckBox
    QCheckBox *oldCheckBox = parent->findChild<QCheckBox*>(object_name);
    if (!oldCheckBox) {
        qWarning() << "CheckBox" << object_name << "not found!";
        return;
    }

    // Сохраняем состояние и политику размера
    bool isChecked = oldCheckBox->isChecked();
    QSizePolicy sizePolicy = oldCheckBox->sizePolicy();
    int stretch = layout->stretch(layout->indexOf(oldCheckBox));

    // Создаем новый Switch
    Switch *customSwitch = new Switch(parent);
    customSwitch->setObjectName(object_name);
    customSwitch->setChecked(isChecked);
    customSwitch->setSizePolicy(sizePolicy);
    customSwitch->setText(oldCheckBox->text()); // Копируем текст

    // Заменяем виджет в layout
    int index = layout->indexOf(oldCheckBox);
    QLayoutItem *item = layout->takeAt(index);
    delete item->widget(); // Удаляем старый QCheckBox
    delete item;

    layout->insertWidget(index, customSwitch, stretch);
}